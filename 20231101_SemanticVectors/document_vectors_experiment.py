...
def generate_data_word2vec(
    ...
    h0_observations = []
    for sentence in h0_documents:
        sentence_encoding_list = [
            model[token] if token in model else np.zeros((300,)) for token in sentence
        ]
        sentence_encoding_matrix = np.array(sentence_encoding_list)
        sentence_encoding_list_sum = np.sum(sentence_encoding_matrix, axis=0)
        h0_observations.append((sentence_encoding_list_sum, 0))
    ...
    for sentence in h1_documents:
        ...
        sentence_encoding_matrix = np.array(sentence_encoding_list)
        sentence_encoding_list_sum = np.sum(sentence_encoding_matrix, axis=0)
        h1_observations.append((sentence_encoding_list_sum, 1))
    ...
    for x_matrix, clase in all_data:
        X.append(x_matrix)
        y.append(clase)
